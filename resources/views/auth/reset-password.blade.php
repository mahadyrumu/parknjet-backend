<x-guest-layout>
    @if ($invalidToken)
        <p class="text-md text-black text-center space-y-1">The token is expired. Please go to<a
                href="{{ env('APP_URL') }}/forgot-password" class="text-blue-600 px-2">Forgot Password</a>page to
            make request again.</p>
    @else
        <form method="POST" action="{{ route('password.store') }}">
            ...
            <div>
                ...
                <x-text-input id="email" class="block mt-1 w-full" type="email" name="user_name" :value="old('user_name', $request->email)"
